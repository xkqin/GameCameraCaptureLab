...
def build_lua_block(config: AppConfig, newline: str = "\n") -> str:
    ...
    lines = [
        ...
        "re9_pose_logger.last_command_issued_at = re9_pose_logger.last_command_issued_at or 0",
        ...
        "        last_command_id = re9_pose_logger.last_command_id or \"\",",
        ...
        "                re9_pose_escape_json(payload.last_command_id),",
        "                re9_pose_escape_json(payload.last_error)))",
        "            f:close()",
        "        elseif json ~= nil and json.dump_file ~= nil then",
        ...
        "            issued_at = tonumber(text:match('\"issued_at\"%s*:%s*([%-%d%.]+)')),",
        ...
        "            trajectory_file = text:match('\"trajectory_file\"%s*:%s*\"([^\"]+)\"')",
        "        }",
        "    end)",
        ...
        "    if (type(frames) ~= \"table\" or #frames < 2) and control.trajectory_file ~= nil and json ~= nil and json.load_file ~= nil then",
        "        local ok_payload, payload = pcall(function() return json.load_file(re9_pose_basename(control.trajectory_file)) end)",
        "        if ok_payload and type(payload) == \"table\" then frames = payload.keyframes or payload end",
        "    end",
        ...
        "    local issued_at = tonumber(control.issued_at) or 0",
        "    if issued_at > 0 and issued_at < (tonumber(re9_pose_logger.last_command_issued_at) or 0) then return end",
        "    if command_id ~= \"\" and command_id == re9_pose_logger.last_command_id then return end",
        "    if issued_at > 0 then re9_pose_logger.last_command_issued_at = issued_at end",
        "    re9_pose_logger.last_command_id = command_id",
        "    if control.command == \"start\" and (control.session_id ~= re9_pose_logger.session_id or not re9_pose_logger.logging) then",
        "        re9_pose_start(control.session_id, control.pose_log_file, control.interval_sec)",
        "    elseif control.command == \"stop\" then",
        "        re9_pose_logger.session_id = control.session_id or re9_pose_logger.session_id",
        "        re9_pose_stop()",
        "    elseif control.command == \"health_check\" then",
        "        re9_pose_write_status()",
