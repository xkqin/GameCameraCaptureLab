...
def one_per_family_trajectories(
    trajectories: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    selected = []
    for family_id in sorted({trajectory["family_id"] for trajectory in trajectories}):
        family = [trajectory for trajectory in trajectories if trajectory["family_id"] == family_id]
        selected.append(family[(len(family) - 1) // 2])
    return selected
...
def main() -> None:
    ...
    family_coverage_smoke = one_per_family_trajectories(trajectories)
    family_coverage_ui = ui_payload(family_coverage_smoke, args.scene_id)
    family_coverage_ui["trajectory_count"] = len(family_coverage_smoke)
    ...
    if len(family_coverage_smoke) > 10:
        (args.output_dir / (
            f"{args.scene_id.replace('.', '_')}_terminalgain_globalmax_interp_"
            f"smoke{len(family_coverage_smoke)}_all_families_low_to_high_ui.json"
        )).write_text(
            json.dumps(family_coverage_ui, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
