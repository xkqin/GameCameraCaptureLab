...
def select_start_anchors(
    ...
    for node_index, node in enumerate(nodes):
        ...
        if xyz_signature in seen_xyz:
            rejected["duplicate_start_xyz"] += 1
            continue
        seen_xyz.add(xyz_signature)
        eligible.append((distance, -node.raw_score, node.node_id, node_index))
    eligible.sort()
    ...
    return starts, {
        ...
        "selection_policy": "all unique XYZ starts inside configured terminal-gain and distance bounds",
...
def draw_xyz_paths(
    ...
    figure.savefig(output_dir / "xyz_paths_all_1000.png", dpi=190, bbox_inches="tight")
    plt.close(figure)
...
def draw_sampled_xyz(output_dir: Path, trajectories: list[dict[str, Any]]) -> None:
    ...
    figure = plt.figure(figsize=(20, 4 * family_count), constrained_layout=True)
...
def draw_distance(output_dir: Path, trajectories: list[dict[str, Any]]) -> None:
    ...
    axis.set_title("Physical cumulative distance vs time (all 1000 candidate curves)")
    axis.set_xlabel("time_sec")
    axis.set_ylabel("physical cumulative distance (m)")
    axis.grid(True, alpha=0.35)
    axis.legend()
    figure.savefig(output_dir / "physical_distance_vs_time_all_1000.png", dpi=190, bbox_inches="tight")
    plt.close(figure)
